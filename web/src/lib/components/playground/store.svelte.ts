...
class PlaygroundStore {
  ...
  desktopFiles = $state<DesktopItem[]>(
    INITIAL_DESKTOP_FILES.map((d) => ({ ...d, file: { ...d.file } })),
  );

  leftPanel = $state({ bucket: BUCKET_NAME, currentPath: "" });
  rightPanel = $state({
    bucket: BUCKET_NAME,
    currentPath: "project-assets/images/",
  });
  ...
  // Profile tabs
  profiles = $state<
    { id: string; name: string; provider: string; connected: boolean }[]
    ...
    demo: {
      ...
      "static-assets": [
        ...
        },
        {
          ...
          lastModified: "2026-04-09T14:00:00Z",
        },
        {
          id: "demo-sa4",
          name: "style.css",
      ...
      logs: [
        {
          id: "demo-lg1",
          name: "access-2026-04.log",
          type: "file",
          size: 8_900_000,
          path: "",
          contentType: "text/plain",
          lastModified: "2026-04-10T06:00:00Z",
        },
        {
          id: "demo-lg2",
          name: "error-2026-04.log",
          type: "file",
          size: 340_000,
          path: "",
          contentType: "text/plain",
  ...
  leftPanelFiles = $derived.by(() => {
    let files = sortFiles(
      filesAtPath(this.bucketFiles, this.leftPanel.currentPath),
      this.sortField,
      this.sortOrder,
    );
    if (this.searchQuery)
  ...
  rightPanelFiles = $derived.by(() => {
    let files = sortFiles(
      filesAtPath(this.bucketFiles, this.rightPanel.currentPath),
      this.sortField,
      this.sortOrder,
    );
    if (this.searchQuery)
  ...
  activeTransferCount = $derived(
    this.transfers.filter((t) => t.status === "active" || t.status === "queued")
      .length,
  );
  ...
  toggleDesktopSelect(fileId: string, additive = false) {
    const next = additive
      ? new Set(this.selectedDesktopFiles)
      : new Set<string>();
    if (next.has(fileId)) next.delete(fileId);
    else next.add(fileId);
  ...
  downloadSelected() {
    const files = this.bucketFiles.filter(
      (f) => this.selectedFiles.has(f.id) && f.type === "file",
    );
  ...
  downloadToDesktop(file: VirtualFile, x?: number, y?: number) {
    ...
    const pos =
      x !== undefined && y !== undefined ? { x, y } : this.nextDesktopSlot();
  ...
  deleteFile(file: VirtualFile, source: DragState["source"]) {
    if (source === "desktop") {
      this.desktopFiles = this.desktopFiles.filter(
        (d) => d.file.id !== file.id,
      );
    } else if (file.type === "folder") {
      // Delete folder and all children whose path starts with this folder's full path
      const folderPrefix = file.path + file.name + "/";
      this.bucketFiles = this.bucketFiles.filter(
        (f) => f.id !== file.id && !f.path.startsWith(folderPrefix),
      );
    } else {
      this.bucketFiles = this.bucketFiles.filter((f) => f.id !== file.id);
    }
  ...
  openContextMenu(
    ...
    y: number,
    target: VirtualFile,
    source: ContextMenuState["source"],
  ) {
